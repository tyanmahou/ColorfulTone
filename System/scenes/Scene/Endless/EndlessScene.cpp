...
{
	using namespace ct;
	using MemoInfo = EndlessScene::MemoInfo;

	MemoInfo g_selectInfo = MemoInfo::Endless;
}
namespace ct
{
	class EndlessScene::Model
	{
	private:
		GameData* m_data;
		Audition m_audition;
	...
	public:
		...
		{
			return m_data->m_endless.getCurrentNotes();
		}
		void init()
		{
		...
		void update()
		{
			...
			if (KeyShift.down()) {
				SoundManager::PlaySe(U"desisionSmall");
				g_selectInfo = g_selectInfo == MemoInfo::Endless
					? MemoInfo::Notes : MemoInfo::Endless;
			}
			m_highSpeed.update(m_data->m_scrollRate);

			// 1曲目でバックまたはバック長押し
			if ((PlayKey::BigBack().down() || PlayKey::SmallBack().down()) && m_data->m_endless.isStart() ||
	...
	EndlessScene::EndlessScene(const InitData& init) :
		ISceneBase(init),
	...
	void EndlessScene::draw() const
	{
		m_view.draw();

		const bool isStart = getData().m_endless.isStart();
		if (!isStart) {
	...
	//関数：drawFadeIn
	//--------------------------------------------------------------------------------
	void EndlessScene::drawFadeIn(double t) const
	{
	...
	//関数：drawFadeOut
	//--------------------------------------------------------------------------------
	void EndlessScene::drawFadeOut(double t) const
	{
		if (getData().m_toScene == SceneName::Main) {
			const MusicData music = getData().m_endless.getCurrentNotes().getMusic();
			this->draw();
	...
	const PlayEndless& EndlessScene::getPlay() const
	{
		return getData().m_endless;
	}

	const HighSpeedDemo& EndlessScene::getHighSpeedDemo() const
	{
		return m_pModel->getHighSpeedDemo();
	}

	const ConfigMain& EndlessScene::getConfig() const
	{
		return m_pModel->getConfig();
	}

	MemoInfo EndlessScene::GetMemoInfo()
	{
		return g_selectInfo;
	}
}
