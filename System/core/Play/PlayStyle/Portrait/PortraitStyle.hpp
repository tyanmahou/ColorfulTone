...
namespace ct
{
	...
	protected:
		...
		void drawComboAndRate(size_t combo, const s3d::Optional<size_t>& totalCombo, float rate, const s3d::Optional<float>& subRate);

		void drawTapEffect(NoteType type, NoteType baseType);
		void drawJudgeEffect(ScoreJudge judge, NoteType type, s3d::int64 diff);
		...
		void drawComboAndRate(size_t combo, const s3d::Optional<size_t>& totalCombo, const s3d::Vec2& comboPos, float rate, const s3d::Optional<float>& subRate, const s3d::Vec2& ratePos) const;
	protected:
